...
`Braid.waves()` groups steps into dependency **waves**: a step is in wave `1 + max(wave of the steps producing its inputs)`, or wave 0 if it depends only on the braid's starting types. Steps in the same wave are independent and the executor runs them **concurrently** (`asyncio.gather`); waves run in order. A purely linear braid is one step per wave, so it executes sequentially exactly as before — branch concurrency only appears when the graph actually forks (e.g. `taxid → traits` and `taxid → disease` from one `name → taxid`). Under the distributed (arq) executor this means independent branch steps dispatch to workers in parallel.
