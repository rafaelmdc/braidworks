...
(the backend `fetch` methods and their `fingerprint`s).
...
This is the novel ~20% the scaffold cannot write for you. Fill in ``fingerprint``
with a real, version-specific string and ``fetch`` with the lookup against the
{{BACKEND}} source, normalizing each result into a ``{{CLASS}}Record``.
"""
...
class {{BACKEND_CLASS}}({{CLASS}}Backend):
    ...
    def __init__(self) -> None:
        self._configured = False  # TODO: set True once the data source is wired

    def is_configured(self) -> bool:
        return self._configured

    def fingerprint(self) -> str:
        # TODO: return a stable, version-specific fingerprint (release tag, dump
        # date, or checksum). Source declared in the spec: {{FINGERPRINT_SOURCE}}.
    ...
    async def fetch(
        ...
        *,
        ...
        # TODO: implement the {{BACKEND}} lookup. Return one {{CLASS}}Record per
        # input query, in order. Set record.found / record.values / record.error.
