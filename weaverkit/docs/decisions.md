...
by reshaping the real `ncbi_weaver` onto the backbone (the migration acted as an
...
- **Shared runtime (maximize reuse).** The domain-neutral 80% of a weaver lives in
  ...
  edit). "Bring your own plumbing" remains available (ncbi_weaver) per the principle
...
dict[str, Any]`). Typed domain records (like ncbi_weaver's `TaxonMatch`) live in
