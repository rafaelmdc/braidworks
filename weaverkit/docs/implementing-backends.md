...
- **`weavers/ncbi_weaver/`** — the *advanced, real-world* one you graduate to: a
...
`weaver_id` may differ from the package name (ncbi_weaver's is `ncbi`); verify
...
- **`kind = "resolver"`** — fuzzy/ambiguous matching (names → ids, like
  `ncbi_weaver`). `fetch` returns `ResolverRecord`s carrying a `MatchStatus`
...
time (e.g. in a metadata table) and read it here — mirror `ncbi_weaver`'s
...
bundled data you point at. `ncbi_weaver` is the worked example:
`build_ncbi_weaver_fixture()` builds a ~6-species SQLite from inline dumps
(`weavers/ncbi_weaver/src/ncbi_weaver/fixture.py`), and its golden uses organisms from that
...
- **Bring your own dispatch/mapper/records.** A rich weaver may implement
  ...
  `MANIFEST` matches the spec and golden passes. `ncbi_weaver` is the worked example:
...
`weavers/ncbi_weaver/src/ncbi_weaver/setup.py` is the worked example (delegates to
