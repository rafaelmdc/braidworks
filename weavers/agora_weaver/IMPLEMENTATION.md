...
- [ ] `src/agora_weaver/backends/local.py` — fill `is_configured`, `fingerprint`, and `fetch` ([#fingerprint](../../weaverkit/docs/implementing-backends.md#fingerprint), [#fetch](../../weaverkit/docs/implementing-backends.md#fetch))

## 2. Verify (definition of done)

- [ ] `make test` — conformance + contract + golden all green
- [ ] `weaverkit verify --spec weaver.spec.toml --package agora_weaver --strict` — no placeholders left, golden runs
- [ ] register the weaver where the app assembles its `WeaverFactory` (see README)
- [ ] after merge, tag the release `agora_weaver-v0.1.0` — `make tags` does this for every bumped package (CI also auto-tags on merge to main)
