...
- [ ] `src/disbiome_weaver/backends/local.py` — fill `is_configured`, `fingerprint`, and `fetch` ([#fingerprint](../../weaverkit/docs/implementing-backends.md#fingerprint), [#fetch](../../weaverkit/docs/implementing-backends.md#fetch))

## 2. Verify (definition of done)

- [ ] `make test` — conformance + contract + golden all green
- [ ] `weaverkit verify --spec weaver.spec.toml --package disbiome_weaver --strict` — no placeholders left, golden runs
