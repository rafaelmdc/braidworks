"""Builders for string_weaver — how the weaver is assembled from its backends.

Two-builder convention (see weaverkit/docs/decisions.md C/D):

- ``build_string_weaver()`` — the ZERO-CONFIG *introspection* builder that
...
def build_string_weaver(**_config: Any) -> BaseWeaver:
    ...
    """
    backends = {
        "api": StringApiBackend(),
    }
    return StringWeaver(backends)


def build_string_weaver_fixture() -> BaseWeaver:
    ...
    """
    from string_weaver.fixture import mock_client

    return StringWeaver({"api": StringApiBackend(client=mock_client())})
...
#
# def build_string_weaver_configured(**config: Any) -> BaseWeaver:
#     backends = {}
