"""Contract tests for string_weaver — execute_batch order/length + cache-key rules.
...
from string_weaver import factory, vocab
...
def _build_weaver():
    ...
    builder = getattr(factory, "build_string_weaver_fixture", None) or factory.build_string_weaver
    return builder()


class TestApiOrder(WeaverOrderContractTests):
    capability_id = "resolve_interactions"
    minimal_outputs = frozenset({"protein.interaction.count"})
    backend = "api"
...
class TestResolveInteractionsCacheFingerprint(CacheFingerprintTests):
    capability = _MANIFEST.capability("resolve_interactions")
