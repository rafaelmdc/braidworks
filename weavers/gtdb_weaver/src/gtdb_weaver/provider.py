"""GtdbWeaverProvider — the Layer 1 conformance wrapper, plus registration.
...
from gtdb_weaver import vocab
from gtdb_weaver.factory import build_gtdb_weaver


class GtdbWeaverProvider:
    """WeaverProvider (Layer 1); delegates to build_gtdb_weaver."""

    weaver_id = vocab.WEAVER_ID

    def build(self, config: Mapping[str, Any]) -> BaseWeaver:
        return build_gtdb_weaver(**dict(config))


def register(factory: WeaverFactory) -> None:
    """Register this weaver's provider so the braider can build "gtdb"."""
    factory.register(GtdbWeaverProvider())
