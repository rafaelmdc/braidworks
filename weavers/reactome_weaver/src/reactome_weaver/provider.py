"""ReactomeWeaverProvider — the Layer 1 conformance wrapper, plus registration.
...
from reactome_weaver import vocab
from reactome_weaver.factory import build_reactome_weaver


class ReactomeWeaverProvider:
    """WeaverProvider (Layer 1); delegates to build_reactome_weaver."""

    weaver_id = vocab.WEAVER_ID

    def build(self, config: Mapping[str, Any]) -> BaseWeaver:
        return build_reactome_weaver(**dict(config))


def register(factory: WeaverFactory) -> None:
    """Register this weaver's provider so the braider can build "reactome"."""
    factory.register(ReactomeWeaverProvider())
