"""BacdiveWeaverProvider — the Layer 1 conformance wrapper, plus registration.
...
class BacdiveWeaverProvider:
    """WeaverProvider (Layer 1); delegates to build_bacdive_weaver."""

    weaver_id = vocab.WEAVER_ID

    def build(self, config: Mapping[str, Any]) -> BaseWeaver:
        return build_bacdive_weaver(**dict(config))


def register(factory: WeaverFactory) -> None:
    """Register this weaver's provider so the braider can build "bacdive"."""
    factory.register(BacdiveWeaverProvider())
