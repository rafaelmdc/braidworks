"""GmrepoWeaverProvider — the Layer 1 conformance wrapper, plus registration.
...
from gmrepo_weaver import vocab
from gmrepo_weaver.factory import build_gmrepo_weaver


class GmrepoWeaverProvider:
    """WeaverProvider (Layer 1); delegates to build_gmrepo_weaver."""

    weaver_id = vocab.WEAVER_ID

    def build(self, config: Mapping[str, Any]) -> BaseWeaver:
        return build_gmrepo_weaver(**dict(config))


def register(factory: WeaverFactory) -> None:
    """Register this weaver's provider so the braider can build "gmrepo"."""
    factory.register(GmrepoWeaverProvider())
