# Contributing to gmrepo_weaver

GMrepo gut-metagenome abundances (taxid -> prevalence + median relative abundance, global and per-phenotype). Source: https://gmrepo.humangut.info (Open (academic)). Kind: `lookup`. Capabilities: `gmrepo.list_abundances`.
...
```bash
weaverkit verify --spec weaver.spec.toml --package gmrepo_weaver --strict
```
...
- **Backend:** add its name to `[weaver].backends`, regenerate, and implement the
  new `src/gmrepo_weaver/backends/<name>.py` (`is_configured` / `fingerprint` / `fetch`).

## Keep the fixture & golden honest

- Golden inputs must resolve in whatever `--strict` runs against (a `build_gmrepo_weaver_fixture()`
  or a configured backend). When the source data changes, bump the backend `fingerprint`
  and refresh the fixture/golden.

## Current outputs

This weaver currently produces: `microbe.abundance.associations`, `microbe.abundance.count`, `microbe.abundance.overview`, `microbe.abundance.phenotype_names`, `microbe.abundance.records`.
