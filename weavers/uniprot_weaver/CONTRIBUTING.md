# Contributing to uniprot_weaver

UniProt protein identity (gene/protein query -> accession + taxid + annotation). Source: https://www.uniprot.org (CC BY 4.0 (UniProt Consortium; cite https://doi.org/10.1093/nar/gkac1052)). Kind: `lookup`. Capabilities: `resolve_protein`.
...
- **Backend:** add its name to `[weaver].backends`, regenerate, and implement the
  new `src/uniprot_weaver/backends/<name>.py` (`is_configured` / `fingerprint` / `fetch`).

## Keep the fixture & golden honest

- Golden inputs must resolve in whatever `--strict` runs against (a `build_uniprot_weaver_fixture()`
