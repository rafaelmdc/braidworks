"""Builders for uniprot_weaver — how the weaver is assembled from its backends.

Two-builder convention (see weaverkit/docs/decisions.md C/D):

- ``build_uniprot_weaver()`` — the ZERO-CONFIG *introspection* builder that
...
from uniprot_weaver.backends.api import UniprotApiBackend
from uniprot_weaver.weaver import UniprotWeaver


def build_uniprot_weaver(**_config: Any) -> BaseWeaver:
    ...
    """
    backends = {
        "api": UniprotApiBackend(),
    }
    return UniprotWeaver(backends)


def build_uniprot_weaver_fixture() -> BaseWeaver:
    ...
    """
    from uniprot_weaver.fixture import mock_client

    return UniprotWeaver({"api": UniprotApiBackend(client=mock_client())})
...
#
# def build_uniprot_weaver_configured(**config: Any) -> BaseWeaver:
#     backends = {}
